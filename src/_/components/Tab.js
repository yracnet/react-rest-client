...
export const Tab = ({ children = [], activeName: init, ...props }) => {
    ...
    let navLinks = children.map(item => {
        ...
        return <span className={'a nav-link ' + active} href={'#' + name} key={name} onClick={e => setActiveName(name)}>{title}</span>
    })
    let navContents = children.map(item => {
        let { name, children } = item.props
        let active = activeName === name ? 'show active' : ''
        return <div className={'tab-pane fade ' + active} key={name}>{children}</div>
    })
    return (
        <div {...props}>
            ...
            <div className="tab-content px-1 py-2">
                {navContents}
            </div>
        </div>
    )
