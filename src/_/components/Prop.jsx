      ...
      <tbody>
        {value?.map((it, ix) => (
          <tr key={ix}>
            ...
            />
            <TDInput
              name={parent + ix + ".value"}
              value={it.value}
              onChange={onChange}
              disabled={disabled}
            />
            <THButtom
              onClick={(e) => onClickRemove(ix)}
              ...
              <table className="table table-sm table-props m-0 p-0">
                <tbody>
                  {it.value.map((it2, ix2) => (
                    <tr key={ix2}>
                      <TDInput
                        name={parent + ix + ".value." + ix2}
                        value={it2}
                        onChange={onChange}
                        disabled={disabled}
                      />
                      <THButtom
                        onClick={(e) => onClickRemoveItem(ix, ix2)}
                  ...
                  <tr>
                    <td></td>
                    <THButtom
                      onClick={(e) => onClickAppendItem(ix)}
...
const THButtom = ({ disabled, ...props }) => {
  return <th width={40}>{disabled ? null : <button {...props} />}</th>;
};
