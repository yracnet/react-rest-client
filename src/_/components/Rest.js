...
const ICONS = ['globe', 'institution', 'rocket', 'send-o', 'hashtag', 'linux', 'android', 'qrcode', 'download', 'upload', 'bomb', 'bug', 'cloud', 'code', 'cube', 'database', 'exchange', 'expeditedssl', 'flash', 'gg', 'link', 'paw', 'pie-chart', 'share-alt', 'sign-in', 'sign-out', 'slack', 'support', 'list-ul', 'server', 'tasks', 'terminal', 'thumb-tack', 'usb', 'user-secret']
        ...
        <div className="media">
            <div className="btn-group" role="group">
                ...
                <div className="dropdown-menu" ref={ref} style={{ width: 550 }}>
                    {ICONS.map(it => (
                        <label key={it} className={'a service-icon ' + (it === value.icon ? 'active' : '')}>
                            <input name={parent + "icon"}
                                value={it}
                                onChange={onChange}
                                type="radio" />
            ...
            <div className="media-body mr-2">
                ...
                <textarea name={parent + "description"}
        ...
        <div className="media">
            ...
            </div>
            {
                ...
                <div className="align-self-center mr-2 text-right">
                    <div className="h5 m-0 font-weight-bold text-danger">{value.method}</div>
        ...
        <div className="input-group mb-3">
            <div className="input-group-prepend">
                <select className="custom-select custom-select-lg font-weight-bold text-danger"
                    ...
                    onChange={onChange}
                    disabled={disabled}>
                    {methods.map(it => <option key={it}>{it}</option>)}
                </select>
            </div>
            <input type="text"
